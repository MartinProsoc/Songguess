...
async function pushLeaderboardEntry(profile) {
    ...
    try {
        ...
        await firestoreDb.collection(LEADERBOARD_COLLECTION).doc(currentUser.uid).set({
...
async function loadLeaderboard(scope) {
    ...
    try {
        ...
        lbList.innerHTML = top.map((entry, idx) => renderLbRow(entry, idx + 1, myUid)).join('');

        const myIndex = myUid ? entries.findIndex(e => e.id === myUid) : -1;
        if (myIndex >= 0 && myIndex >= LB_PAGE_SIZE) {
            lbMyRow.innerHTML = renderLbRow(entries[myIndex], myIndex + 1, myUid);
