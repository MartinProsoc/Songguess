...
const RANK_TITLES = [
    ...
    { min: 20, title: "Songuess Ikon", icon: "🌟" },
];
...
shareResultBtn.addEventListener('click', async () => {
    const header = config.isDaily
        ? `Songuess Napi Kihívás 🎧 (${CATEGORY_LABELS[config.category]})`
        : `Songuess Egyéni Játék 🎧 (${CATEGORY_LABELS[config.category] || config.category})`;
