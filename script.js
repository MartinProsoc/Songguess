...
async function pushCloudProfile(uid, profile) {
    ...
    try { await firestoreDb.collection('users').doc(uid).set(profile); } catch (e) {}
}
...
let config = { username: "Játékos", rounds: 5, timeLimit: 0, volume: 0.5, category: "all", mode: "classic", hardcore: false, isDaily: false, isMultiplayer: false };
...
const multiplayerMenuScreen = document.getElementById('multiplayer-menu-screen');
const roomLobbyScreen = document.getElementById('room-lobby-screen');

const ALL_SCREENS = [homeScreen, setupScreen, gameScreen, statsScreen, profileScreen, leaderboardScreen, multiplayerMenuScreen, roomLobbyScreen];
...
// ==========================================
// 7. JÁTÉK INDÍTÁSA (Egyéni, Napi, Quickplay)
// ==========================================
...
async function startRound() {
    ...
    const rng = ((config.isDaily || config.isMultiplayer) && dailyRngInstance) ? dailyRngInstance : Math.random;
...
function endRound(isWin, msgText, isFlawless = false) {
    ...
    // Multiplayer pontszám felszinkronizálása
    if (config.isMultiplayer && currentRoomId && currentUser) {
        firestoreDb.collection('rooms').doc(currentRoomId).update({
            [`players.${currentUser.uid}.score`]: firebase.firestore.FieldValue.increment(isWin ? 1 : 0)
    ...
    if (config.isMultiplayer && currentRoundNum >= config.rounds) {
        nextRoundBtn.textContent = "Végeredmény megtekintése";
    } else {
...
nextRoundBtn.addEventListener('click', async () => {
    ...
    if (currentRoundNum >= config.rounds) {
        if (config.isMultiplayer) {
            // Multiplayer játék vége: Host jelzi az adatbázisban, és mindenkit visszadob a váróba
            if (isHost && currentRoomId) {
                firestoreDb.collection('rooms').doc(currentRoomId).update({ status: 'finished' });
            }
            document.getElementById('room-waiting-msg').textContent = "A játék véget ért! Végeredmény felül.";
            document.getElementById('start-room-btn').classList.add('hidden');
...
function showStats() {
    ...
    if (config.isDaily) sessionXp += 50; 
...
// ==========================================
// 15. RANGLISTA LEKÉRÉSE
// ==========================================
leaderboardBtn.addEventListener('click', async () => {
    showScreen(leaderboardScreen);
    leaderboardList.innerHTML = '<p class="text-secondary mt-3">Ranglista betöltése...</p>';
    
    if (!firestoreDb) {
        leaderboardList.innerHTML = '<p class="auth-error-msg mt-3">A ranglista betöltéséhez jelentkezz be a profilodnál!</p>';
        return;
    }

    try {
        const snapshot = await firestoreDb.collection('users').orderBy('xp', 'desc').limit(50).get();
        leaderboardList.innerHTML = '';
        
        if (snapshot.empty) {
            leaderboardList.innerHTML = '<p class="text-secondary mt-3">Még nincsenek játékosok.</p>';
            return;
        }
        ...
        snapshot.forEach(doc => {
            ...
            row.innerHTML = `
                <span style="display:flex; align-items:center; gap:10px;">
                    <strong style="color:var(--muted); width:20px;">#${rank}</strong> 
                    <span>${levelInfo.title.icon} ${data.username || 'Névtelen'}</span>
                </span>
                <strong class="gold-text">${data.xp || 0} XP</strong>
            `;
            leaderboardList.appendChild(row);
            rank++;
        });
    } catch (e) {
        leaderboardList.innerHTML = `<p class="auth-error-msg mt-3">Hiba a betöltéskor: ${e.message}</p>`;
    }
});
leaderboardBackBtn.addEventListener('click', () => showScreen(homeScreen));

// ==========================================
// 16. MULTIPLAYER LOBBY RENDSZER
// ==========================================
...
multiplayerLobbyBtn.addEventListener('click', () => {
    if (!currentUser) {
        alert("A Multiplayer módhoz be kell jelentkezned a profilodnál (kis fülhallgató ikon a jobb felső sarokban)!");
        return;
    }
    document.getElementById('multiplayer-error-msg').textContent = '';
    showScreen(multiplayerMenuScreen);
});

document.getElementById('multiplayer-back-btn').addEventListener('click', () => showScreen(homeScreen));
...
// Szoba létrehozása (Host)
document.getElementById('create-room-btn').addEventListener('click', async () => {
    const code = Math.random().toString(36).substring(2, 6).toUpperCase();
    const cat = document.getElementById('multiplayer-category').value;
    ...
    try {
        await firestoreDb.collection('rooms').doc(code).set({
            ...
            seed: seed,
            players: {
                [currentUser.uid]: { name: profile.username || 'Host', score: 0 }
            }
        });
        ...
        document.getElementById('multiplayer-error-msg').textContent = "Hiba a szoba létrehozásakor. Ellenőrizd a Firestore szabályokat.";
    }
});

// Szobához csatlakozás (Client)
document.getElementById('join-room-btn').addEventListener('click', () => {
    ...
    if (code.length === 4) {
        ...
        document.getElementById('multiplayer-error-msg').textContent = "A kódnak 4 karakterből kell állnia.";
    }
});
...
function joinRoomLogic(code) {
    ...
    if (!isHost) {
        firestoreDb.collection('rooms').doc(code).update({
            [`players.${currentUser.uid}`]: { name: profile.username || 'Játékos', score: 0 }
        }).catch(e => {
            alert("Nem található ilyen szoba, vagy már lezárult!");
            showScreen(multiplayerMenuScreen);
        });
    }
    ...
    unsubscribeRoom = firestoreDb.collection('rooms').doc(code).onSnapshot(doc => {
        if (!doc.exists) {
        ...
        const data = doc.data();
        document.getElementById('room-category-display').textContent = 'Kategória: ' + (CATEGORY_LABELS[data.category] || data.category);
        ...
        sortedPlayers.forEach(p => {
            list.innerHTML += `
                <div class="stat-row">
                    ...
                    <strong class="gold-text">${p.score} pont</strong>
                </div>`;
        });

        if (isHost && data.status === 'waiting') {
            document.getElementById('start-room-btn').classList.remove('hidden');
            document.getElementById('room-waiting-msg').classList.add('hidden');
        } else {
            document.getElementById('start-room-btn').classList.add('hidden');
            document.getElementById('room-waiting-msg').classList.remove('hidden');
        }

        if (data.status === 'playing' && document.getElementById('game-screen').classList.contains('active') === false) {
            startMultiplayerGame(data.category, data.seed);
        }
        
        if (data.status === 'finished' && document.getElementById('room-lobby-screen').classList.contains('active') === false) {
             document.getElementById('room-waiting-msg').textContent = "A játék véget ért! Végeredmény felül.";
             showScreen(roomLobbyScreen);
        }
    });
...
document.getElementById('start-room-btn').addEventListener('click', () => {
    firestoreDb.collection('rooms').doc(currentRoomId).update({ status: 'playing' });
});

async function startMultiplayerGame(category, seed) {
    const profile = loadProfile();
    config.username = profile.username || "Játékos";
    ...
    dailyRngInstance = mulberry32(seed); 
    resetSessionState();
    
    hardcoreBadge.classList.add('hidden');
    dailyModeBadge.classList.remove('hidden');
    dailyModeBadge.textContent = `👥 MULTIPLAYER · ${currentRoomId}`;
    skipBtn.classList.remove('hidden');
