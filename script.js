...
async function getRandomSongFromiTunes(category, excludeIds = new Set(), rng = Math.random) {
    ...
    for (const term of shuffledTerms) {
        ...
        try {
            ...
            // Háromlépcsős egyezés-keresés:
            //   1) PONTOS: a kulcsszó az előadó mezőben teljes, önálló szegmensként szerepel
            //      (artistFieldMatchesTerm) — ez a legmegbízhatóbb, konkrét előadóknál
            //      (pl. "curtis", "vini") ezt akarjuk.
            //   2) LAZA: ha nincs pontos egyezés, a kulcsszó valahol az előadó VAGY a cím
            //      mezőben szerepel részletként. Ez kell az olyan generikus (nem egy konkrét
            //      személyre utaló) kulcsszavaknak, mint "league of legends" vagy "top hits",
            //      ahol nincs is egyetlen "helyes" előadó — ott bőven elég, ha a kifejezés
            //      valahol felbukkan.
            //   3) Ha SEM pontos, SEM laza egyezés nincs, ez a kulcsszó valójában nincs jelen
            //      az iTunes katalógusában (pl. az előadó nincs digitálisan forgalmazva —
            //      pontosan ez történt a "fura csuka" kulcsszóval, ami egy teljesen más,
            //      véletlenül hasonló nevű előadót hozott be). Ilyenkor NEM adunk vissza egy
            //      találomra kiválasztott, kapcsolódás nélküli számot, hanem másik kulcsszóval
            //      próbálkozunk (continue).
            const artistMatches = rawTracks.filter(track => artistFieldMatchesTerm(track.artistName, term));
            let validTracks = artistMatches;
