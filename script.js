...
const CATEGORY_TERMS = {
    // A "fura csuka" kifejezés ki lett véve — élőben leteszteltem (lásd a beszélgetés
    // jegyzeteit), és az iTunes-on gyakorlatilag NINCS érdemi találat rá: a keresés vagy
    // 0 eredményt adott, vagy egy teljesen más, véletlenül hasonló nevű előadót (Levente
    // Csuka feat. ItsErikson) hozott be a régi "ha nincs pontos egyezés, adj vissza bármit"
    // visszaesési logikán keresztül — innen jött a bejelentett "erickson" rejtély.
    magyar: ['azahriah', 'desh', 'halott penz', 'valmar', 'wellhello', 'beton hofi', 'krubi', 'dzsudlo', 'majka', 'punnany massif', 'kowalsky meg a vega', 'vini', 'bagossy brothers company'],
    magyar_2020: ['azahriah', 'desh', 'valmar', 'vini', 'wellhello', 't. danny', 'bagossy brothers company', 'missh', 'byealex es a slepp', 'curtis'],
...
// Ezek a kulcsszavak NEM egy konkrét előadó nevét jelentik, hanem egy általános, hangulati
// keresőkifejezést (pl. "top hits", "league of legends") — itt nincs egyetlen "helyes
// előadó", akit védeni kellene egy névtévesztéstől, ezért ezekre KIHAGYJUK a szigorú/laza
// szöveg-egyezést, és egyszerűen az iTunes saját relevancia-rangsorolt nyers találati
// listájából válogatunk. E nélkül pont az ellenkezője történne annak, amit a szigorú
// egyezés eredetileg megoldani hivatott volt: élőben leteszteltem, hogy a "summer hits"
// kulcsszóra 30 találatból mindössze 3, egy homályos zenei könyvtárból származó, szó
// szerint "Summer Hits" című szám ("Mazes", "Magazine Beach") maradt volna a laza egyezés
// után — miközben 27 ismert, releváns szám (Calvin Harris, Ed Sheeran stb.) kiesett volna,
// mert a CÍMÜK nem tartalmazza szó szerint a "summer hits" kifejezést. Emiatt ez a két
// homályos szám jött ki feltűnően gyakran a "Mindegyik keverve" kategóriában.
const GENERIC_SEARCH_TERMS = new Set([
    'pop hit 2024', 'top hits', 'rock classic', 'dance music', 'summer hits',
    'league of legends', 'c418 minecraft', 'fifa soundtrack', 'undertale soundtrack', 'zelda soundtrack'
]);

// ==========================================
// 1b. ELGÉPELÉS-TŰRŐ KERESÉS (a tipp-autocomplete-hoz)
// ==========================================
// Egy hiányzó/elcserélt betű (pl. "azariah" a helyes "azahriah" helyett) az iTunes saját
// keresésén simán ELVESZIK — nem egy rangsorolási probléma (mint korábban a "valencia"),
// hanem az, hogy a helyes előadó találatai EGYÁLTALÁN nem szerepelnek a nyers válaszban.
// Élőben leteszteltem: "azariah"-ra 15 találatból egy sem volt a magyar Azahriah-tól, sem
// "azahriah"-ra viszont mind helyes volt. Mivel már van egy saját, kurátorolt előadó-
// listánk (CATEGORY_TERMS — pontosan ezekkel a nevekkel keresünk kör-indításkor is), ezt
// használjuk "szótárként": ha a beírt szöveg csak 1-2 betűnyire van egy ismert névtől, azt
// tekintjük a szándékolt keresésnek, és AZZAL is lekérdezünk, a begépelt szöveg mellett.
const KNOWN_ARTIST_TERMS = [...new Set(
    Object.values(CATEGORY_TERMS).flat().filter(term => !GENERIC_SEARCH_TERMS.has(term))
)];
...
// Ha a beírt szöveg (vagy annak eleje, hosszabb keresés esetén) elég közel van egy ismert
// előadónévhez, visszaadja azt — különben null-t. Rövidebb szónál (≤6 karakter) csak 1
// betűnyi eltérést engedünk, hosszabbnál 2-t, hogy ne javítson túl agresszíven teljesen
// más, rövid szavakat.
function findTypoCorrectedTerm(query) {
    const q = query.trim().toLowerCase();
    if (q.length < 4) return null;
    let best = null;
    let bestDist = Infinity;
    for (const term of KNOWN_ARTIST_TERMS) {
        if (Math.abs(term.length - q.length) > 2) continue; // gyors kizárás — a Levenshtein
        // úgyis elutasítaná, ha a hosszkülönbség önmagában nagyobb, mint a megengedett táv.
...
// Ugyanaz a szám gyakran TÖBB, különböző trackId-vel is szerepel az iTunes katalógusában
// (kislemez + album + válogatáslemez + remaster ugyanarra a dalra) — a puszta trackId-alapú
// kizárás emiatt nem ismeri fel ismétlődésnek, ha egy már meghallgatott szám egy MÁSIK
// kiadásban jön vissza. Ezért az "előadó - cím" (ékezet-/kis-nagybetű-független) normalizált
// kulcsot is a kizárási halmazba tesszük — lásd getRandomSongFromiTunes lent.
function normalizeSongIdentity(artist, title) {
    return `${(artist || '').toLowerCase().trim()}||${(title || '').toLowerCase().trim()}`;
}

// Generikus kifejezésekre (pl. "dance music") az iTunes gyakran ismeretlen előadós, gyűjteményes
// "Workout Mix"/"Cardio Mix" jellegű fitnesz-válogatásokat ad vissza — ezek szinte kitalálhatatlan,
// generikus DJ-összeállítások, nem valódi számok, ezért mindenhonnan kizárjuk őket, függetlenül
// attól, épp melyik kulcsszóra jöttek be.
const WORKOUT_MIX_RE = /workout|cardio mix|spinning class|fitness mix|gym mix|aerobic|zumba|power mix|running mix|jogging mix/i;
function isWorkoutMixTrack(track) {
    return WORKOUT_MIX_RE.test(`${track.artistName || ''} ${track.collectionName || ''} ${track.trackName || ''}`);
}

// NEHÉZSÉGI SZINT — mivel (ahogy korábban megbeszéltük) az iTunes API nem ad vissza semmiféle
// népszerűségi/lejátszásszám-adatot, itt a legjobb elérhető, ŐSZINTE proxy-jelet használjuk:
// egy szám akkor "Ismert", ha szerepel egy olyan albumon, amit a kiadó kifejezetten
// "Greatest Hits"/"Best Of"/"Essential" stb. néven adott ki — ezeket a válogatásokat a
// kiadó/előadó saját maga állítja össze a legismertebb számaiból, tehát ez egy valódi
// (nem kitalált) jelzés, nem pusztán találgatás. FONTOS KORLÁT: ez sem tökéletes — egy
// vadonatúj sláger, aminek még nincs válogatásalbuma, tévesen "Kemény"-nek számítana.
const HITS_COMPILATION_RE = /greatest hits|the best of|the very best|essential|number ones|no\.?\s?1('?s)?|top hits|hits collection|singles collection|the collection|icon\b/i;
function applyDifficultyFilter(tracks, difficulty) {
    if (!difficulty || difficulty === 'any') return tracks;
    const filtered = tracks.filter(t => {
        const isHitsCompilation = HITS_COMPILATION_RE.test(t.collectionName || '');
        return difficulty === 'easy' ? isHitsCompilation : !isHitsCompilation;
    });
    // Ha a szűrés kiürítené a listát (pl. minden találat válogatásalbumon van, vagy
    // egyiken sincs), inkább a teljes, szűretlen listából válogatunk, minthogy egyáltalán
    // ne találjunk számot ehhez a kulcsszóhoz.
    return filtered.length > 0 ? filtered : tracks;
}

async function getRandomSongFromiTunes(category, excludeIds = new Set(), rng = Math.random, difficulty = 'any') {
    ...
    for (const term of shuffledTerms) {
        ...
        try {
            ...
            const rawTracks = data.results.filter(track =>
                track.previewUrl
                && !excludeIds.has(track.trackId)
                && !excludeIds.has(normalizeSongIdentity(track.artistName, track.trackName))
                && !isWorkoutMixTrack(track)
            );
            ...
            if (GENERIC_SEARCH_TERMS.has(term.toLowerCase())) {
                // Generikus hangulati kifejezés — nincs "helyes előadó", bízunk az iTunes
                // saját relevancia-rangsorolásában, nem szűrünk tovább szöveg-egyezésre.
                validTracks = rawTracks;
            } else {
                // Háromlépcsős egyezés-keresés konkrét ELŐADÓ-kulcsszavakhoz:
                //   1) PONTOS: a kulcsszó az előadó mezőben teljes, önálló szegmensként
                //      szerepel (artistFieldMatchesTerm) — ez a legmegbízhatóbb, konkrét
                //      előadóknál (pl. "curtis", "vini") ezt akarjuk.
                //   2) LAZA: ha nincs pontos egyezés, a kulcsszó valahol az előadó VAGY a cím
                //      mezőben szerepel részletként (pl. egy franchise-hoz kötött, nem
                //      önálló-előadós szám esetén).
                //   3) Ha SEM pontos, SEM laza egyezés nincs, ez a kulcsszó valójában nincs
                //      jelen az iTunes katalógusában (pl. az előadó nincs digitálisan
                //      forgalmazva — pontosan ez történt a "fura csuka" kulcsszóval, ami egy
                //      teljesen más, véletlenül hasonló nevű előadót hozott be). Ilyenkor NEM
                //      adunk vissza egy találomra kiválasztott, kapcsolódás nélküli számot,
                //      hanem másik kulcsszóval próbálkozunk (continue).
                validTracks = rawTracks.filter(track => artistFieldMatchesTerm(track.artistName, term));
                if (validTracks.length === 0) {
                    const termLower = term.toLowerCase();
                    validTracks = rawTracks.filter(track =>
                        (track.artistName || '').toLowerCase().includes(termLower) ||
                        (track.trackName || '').toLowerCase().includes(termLower)
                    );
                }
                if (validTracks.length === 0) {
                    console.warn(`"${term}" kulcsszóhoz nem található érdemi (sem pontos, sem laza) egyezés, próbálkozás másikkal...`);
                    continue;
                }
            }

            validTracks = applyDifficultyFilter(validTracks, difficulty);
...
async function fetchITunesRaw(term) {
    try {
        const response = await fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(term)}&entity=song&limit=25&country=${ITUNES_COUNTRY}`);
        const data = await response.json();
        return data.results || [];
    } catch (error) {
        return [];
...
async function searchiTunes(query) {
    ...
    if (!q) return [];
    try {
        // Ha a begépelt szöveg csak 1-2 betűnyire van egy ismert előadónévtől (pl. "azariah"
        // a "azahriah" helyett), AZZAL is lekérdezünk, párhuzamosan a szó szerint begépelt
        // szöveggel — mert a hiányzó/elcserélt betű miatt a helyes előadó találatai simán
        // hiányozhatnak a nyers iTunes-válaszból (ez nem rangsorolási, hanem valódi
        // "nincs is a nyers listában" probléma, lásd findTypoCorrectedTerm().
        const correctedTerm = findTypoCorrectedTerm(q);
        const fetches = [fetchITunesRaw(query)];
        if (correctedTerm) fetches.push(fetchITunesRaw(correctedTerm));
        ...
        resultSets.forEach((results, setIdx) => {
            const fromCorrectedTerm = setIdx === 1; // a 2. lekérdezés jön az elgépelés-javításból
            results.forEach((track, idx) => {
                ...
                // Erős bónusz az elgépelés-javított kifejezésből jött találatoknak — ha idáig
                // eljutottunk, nagy eséllyel pont ezt az előadót kereste a felhasználó.
                if (fromCorrectedTerm) score += 60;
                scored.push({ track, score, idx: setIdx * 1000 + idx });
            });
        });
...
function defaultProfile() {
    return {
        ...
        // Zenei ízlésprofil: kategória-kulcsonként { correct, total } — csak azokból a
        // menetekből frissül, ahol PONTOSAN egy kategória volt kiválasztva (lásd showStats /
        // multiplayer.js renderMpResults), hogy egyértelmű legyen a hozzárendelés.
        categoryStats: {}
    };
}
...
// Kategóriánként, mezőnként a nagyobbikat tartjuk meg — ugyanaz az elv, mint a többi
// számlálónál (Math.max), hogy ismételt szinkronizálás se duplázza a haladást.
function mergeCategoryStats(localStats, cloudStats) {
    const result = {};
    const allKeys = new Set([...Object.keys(localStats || {}), ...Object.keys(cloudStats || {})]);
    allKeys.forEach(key => {
        const l = (localStats && localStats[key]) || { correct: 0, total: 0 };
        const c = (cloudStats && cloudStats[key]) || { correct: 0, total: 0 };
        result[key] = { correct: Math.max(l.correct || 0, c.correct || 0), total: Math.max(l.total || 0, c.total || 0) };
    });
    return result;
}
...
function mergeProfiles(local, cloud) {
    ...
    const merged = {
        ...
        categoryStats: mergeCategoryStats(local.categoryStats, cloud.categoryStats)
    };
...
// FIGYELEM: néhány itteni gyök (pl. "anal", "cum", "prick", "wank", "sex") TUDATOSAN
// ütközik pár ártatlan szóval is (pl. "banális", "analízis", "kanális", "document",
// "circumstance", "prickly", "Wankel", "unisex", "sexagenarian") — ezeket direkt kérésre
// tettük vissza a listába a szigorúbb szűrés kedvéért, a ritka félrefogásokat (valaki pont
// ilyen nevet szeretne) vállalva a szélesebb lefedettségért cserébe.
const USERNAME_DENYLIST = [
    // durva/trágár szavak (magyar) — a gyökér-alakok (pl. "bazd", "basz") a ragozott
    // változatokat is elkapják (bazdmeg, baszd meg, baszod, baszik, elbaszott, kibaszott,
    // stb.), mert a normalizálás előbb kiszedi a szóközöket/írásjeleket.
    'kurva', 'geci', 'genyo', 'picsa', 'fasz', 'buzi', 'koszfej', 'kocsog', 'faszfej', 'szopjal', 'anyad',
    'bazd', 'basz', 'segg', 'pina', 'punci', 'ribanc', 'kuss', 'kreten',
    // explicit szexuális kifejezések (magyar)
    'szex', 'porno', 'vagina', 'penis', 'anal',
    ...
    'fuck', 'shit', 'bitch', 'nigger', 'nigga', 'faggot', 'fag', 'retard', 'whore', 'slut',
    'rape', 'nazi', 'hitler', 'cunt', 'asshole', 'dick', 'pussy', 'bastard', 'prick', 'wank', 'cum',
    'tranny', 'dyke', 'chink', 'kike',
    // "spic" (ütközik: spice, spicy) és "hoe" (ütközik: shoe, shoemaker) — a leggyakoribb
    // szavakkal ütköző két gyök a listában, ugyanazon szigorúbb-szűrés-elve alapján bent.
    'spic', 'hoe',
    // explicit szexuális kifejezések (angol)
    'sex', 'porn', 'tits', 'boobs', 'douche', 'twat', 'jizz', 'milf', 'dilf', 'thot', 'slag'
];
...
const SKIP_COOLDOWN_SESSION_THRESHOLD = 3; // ennyi Kihagyás után lép életbe a HOSSZABB cooldown
const SKIP_COOLDOWN_SECONDS = 3;
// A küszöb ALATT is kell egy rövid, KÖTELEZŐ szünet két Kihagyás-kattintás között — enélkül
// a küszöbig terjedő pár Kihagyást egyetlen pillanat alatt, azonnali kattintgatással el
// lehetett "költeni", végigugorva a hangrészlet-fokozatokon (0.5mp -> 1mp -> 2mp -> ...)
// szinte azonnal, ami multiplayerben tisztességtelen előnyt adott (ő máris a leghosszabb
// részletet hallhatta, amíg mások épp csak az elsőt).
const SKIP_MIN_GAP_MS = 900;
...
// ==========================================
// 3f. KÖR-INDÍTÁS VISSZASZÁMLÁLÓ ("3... 2... 1...")
// ==========================================
// A "pittyenés" hangokat a Web Audio API-val generáljuk egyszerű szinusz-hangokból, nem
// beágyazott hangfájlból — így nincs szükség licencelt/keresett hangeffekt-mintára, és a
// fájlméret is minimális marad. Ugyanezt a mechanikát a multiplayer.js is használja (saját
// overlay-elemekkel), mindkettő ezt a megosztott függvényt hívja.
let sgAudioCtx = null;

// Mobil böngészők (elsősorban iOS Safari) csak KÖZVETLEN felhasználói gesztusra (érintés
// vagy kattintás) engedik elindítani/feloldani a Web Audio kontextust. A visszaszámláló
// viszont csak azután akar hangot lejátszani, hogy a kör száma megérkezett az iTunes-tól
// (egy "await"-tel elválasztott, aszinkron lépés) — ezt a böngésző már NEM tekinti közvetlenül
// a felhasználói érintéshez kötöttnek, ezért csendben letiltja a hangot (a playCountdownTone
// try/catch-je meg is eszi a hibát, így ez láthatatlanul hiúsul meg). A megoldás: az oldal
// ELSŐ érintésekor/kattintásakor, még szinkron módon, azonnal létrehozzuk és feloldjuk a
// hangkontextust — mire a visszaszámláló ténylegesen hangot akarna adni, az már kész áll.
function unlockAudioContext() {
    try {
        const AudioCtxClass = window.AudioContext || window.webkitAudioContext;
        if (!AudioCtxClass) return;
        if (!sgAudioCtx) sgAudioCtx = new AudioCtxClass();
        if (sgAudioCtx.state === 'suspended') sgAudioCtx.resume().catch(() => {});
    } catch (e) { /* no-op */ }
}
document.addEventListener('touchstart', unlockAudioContext, { once: true, passive: true });
document.addEventListener('click', unlockAudioContext, { once: true });

function playCountdownTone(frequency, duration) {
    try {
        const AudioCtxClass = window.AudioContext || window.webkitAudioContext;
        if (!AudioCtxClass) return;
        if (!sgAudioCtx) sgAudioCtx = new AudioCtxClass();
        if (sgAudioCtx.state === 'suspended') sgAudioCtx.resume().catch(() => {});
        const osc = sgAudioCtx.createOscillator();
        const gain = sgAudioCtx.createGain();
        osc.type = 'sine';
        osc.frequency.value = frequency;
        gain.gain.setValueAtTime(0.18, sgAudioCtx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.001, sgAudioCtx.currentTime + duration);
        osc.connect(gain);
        gain.connect(sgAudioCtx.destination);
        osc.start();
        osc.stop(sgAudioCtx.currentTime + duration);
    } catch (e) { /* hangtalan visszaesés — a visszaszámláló hang nélkül is működik */ }
}

const ROUND_COUNTDOWN_STEPS = ['3', '2', '1', '🎵'];
const ROUND_COUNTDOWN_STEP_MS = 550;

// Lejátssza a "3... 2... 1... 🎵" visszaszámlálót a megadott overlay/szám-elemen, majd
// meghívja onComplete-et. A kör vezérlői (Lejátszás/Kihagyás/Tippelés) ezalatt zárolva
// maradnak a hívó oldalán — ez a függvény csak magát a vizuális/hang-effektet vezényli.
function playRoundStartCountdown(overlayEl, numberEl, onComplete) {
    if (!overlayEl || !numberEl) { onComplete(); return; }
    overlayEl.classList.remove('hidden');
    let i = 0;
    function showNext() {
        if (i >= ROUND_COUNTDOWN_STEPS.length) {
            overlayEl.classList.add('hidden');
            onComplete();
            return;
        }
        const isLast = i === ROUND_COUNTDOWN_STEPS.length - 1;
        numberEl.textContent = ROUND_COUNTDOWN_STEPS[i];
        numberEl.classList.remove('pulse');
        void numberEl.offsetWidth; // reflow-kényszerítés, hogy az animáció újrainduljon
        numberEl.classList.add('pulse');
        playCountdownTone(isLast ? 660 : 440, isLast ? 0.3 : 0.14);
        i++;
        setTimeout(showNext, ROUND_COUNTDOWN_STEP_MS);
    }
    showNext();
}

// ==========================================
// 3g. VÁLASZ-FELFEDÉS ANIMÁCIÓ (kör vége — mind solo, mind multiplayer használja)
// ==========================================
// outcome: 'flawless' | 'win' | 'lose'. A coverWrapEl a borítót körülvevő (position:relative)
// konténer — ide kerülnek a flawless-szikrák —, a shakeTargetEl pedig az az elem, ami rossz
// válasznál megrázkódik (solo: a teljes modal-box, multiplayer: a reveal-kártya).
function triggerRevealAnimation(coverEl, coverWrapEl, shakeTargetEl, outcome) {
    if (coverEl) {
        coverEl.classList.remove('reveal-pop');
        void coverEl.offsetWidth; // reflow-kényszerítés, hogy az animáció újrainduljon
        coverEl.classList.add('reveal-pop');
    }
    if (outcome === 'lose' && shakeTargetEl) {
        shakeTargetEl.classList.remove('reveal-shake');
        void shakeTargetEl.offsetWidth;
        shakeTargetEl.classList.add('reveal-shake');
    }
    if (outcome === 'flawless' && coverWrapEl) {
        spawnRevealSparkles(coverWrapEl);
    }
}

function spawnRevealSparkles(wrapEl) {
    const emojis = ['✨', '⭐', '🌟', '💫'];
    const count = 8;
    for (let i = 0; i < count; i++) {
        const span = document.createElement('span');
        span.className = 'reveal-sparkle';
        ...
        span.style.left = '50%';
        span.style.top = '50%';
        span.style.animationDelay = `${(Math.random() * 0.15).toFixed(2)}s`;
        wrapEl.appendChild(span);
        setTimeout(() => span.remove(), 1300);
    }
}

// Kis "pattanás" visszajelzés bármilyen elemre (pl. vágólapra-másolás gombokra sikeres
// másoláskor) — a meglévő .combo-pop animációt (popIn keyframe) újraindítja.
function bouncePop(el) {
    if (!el) return;
...
// Rezgés (haptika) mobilon — a navigator.vibrate() asztali böngészőkön/iOS Safariban
// egyszerűen nem létezik vagy nem csinál semmit, ezért biztonságos mindenhol meghívni,
// nincs szükség kapcsolgatásra platform szerint.
...
const roundStartCountdownOverlay = document.getElementById('round-start-countdown');
const roundStartCountdownNumber = document.getElementById('round-start-countdown-number');
...
const tasteProfileArea = document.getElementById('taste-profile-area');
...
const friendsScreenRef = document.getElementById('friends-screen');
const ALL_SCREENS = [homeScreen, setupScreen, gameScreen, statsScreen, profileScreen, leaderboardScreen, friendsScreenRef];
...
// ==========================================
// PWA TELEPÍTÉS — házon belüli "Letöltés" gomb
// ==========================================
// A böngésző saját "⋮ menü -> Telepítés" útja sokak számára túl rejtett — ehelyett egy jól
// látható gombot adunk a főképernyőre. Chrome/Edge/Android a "beforeinstallprompt" eseménnyel
// jelzi, hogy telepíthető az app — ezt elfogjuk (preventDefault), és MI indítjuk el a natív
// telepítő ablakot a gombra kattintva. Safari (iPhone/iPad) ezt EGYÁLTALÁN NEM támogatja —
// ott nincs programozott telepítési mód, csak a Megosztás -> "Kezdőképernyőhöz adás" kézi
// lépés, ezért ott gombkattintásra egy rövid útmutatót mutatunk telepítés helyett.
...
function refreshInstallBtnVisibility() {
    ...
    if (isRunningStandalone()) { installAppBtn.classList.add('hidden'); return; }
    installAppBtn.classList.toggle('hidden', !deferredInstallPrompt && !isIOSDevice());
}
...
if (installAppBtn) {
    installAppBtn.addEventListener('click', async () => {
        ...
        if (isIOSDevice()) {
            alert('iPhone/iPad-en (Safari) ez a lépés kézi — automatikus telepítést a rendszer nem enged:\n\n1) Koppints a Megosztás gombra (a □ ikon, felfelé mutató nyíllal).\n2) Görgess le, és válaszd: "Kezdőképernyőhöz adás".\n\nFontos: ez csak Safariban működik, más böngészőben (pl. Chrome) iPhone-on nem érhető el.');
        }
    });
}
...
dailyShareAgainBtn.addEventListener('click', async () => {
    ...
    if (ok) bouncePop(dailyShareAgainBtn);
...
startGameBtn.addEventListener('click', async () => {
    ...
    config.practiceMode = practiceInput.checked;
...
dailyStartBtn.addEventListener('click', async () => {
    ...
    config.practiceMode = false; // a Napi Kihívás mindig számít — nem lehet gyakorló módban
...
async function startRound() {
    ...
    currentSong = await getRandomSongFromiTunes(config.category, usedSongIds, rng, config.difficulty);
    ...
    usedSongIds.add(normalizeSongIdentity(currentSong.artist, currentSong.title));
    ...
    // A vezérlők (Lejátszás/Kihagyás/Tippelés/beviteli mező) egyelőre ZÁROLVA maradnak — a
    // "3... 2... 1..." visszaszámláló végén nyílnak csak meg (finalizeRoundStart), hogy a
    // játékos ne tudjon előnyt szerezni a visszaszámlálás alatt.
    ...
    playRoundStartCountdown(roundStartCountdownOverlay, roundStartCountdownNumber, finalizeRoundStart);
}
...
function finalizeRoundStart() {
    ...
    choiceInputArea.querySelectorAll('.choice-btn').forEach(b => b.disabled = false);
...
function triggerSkipCooldown() {
    ...
    if (sessionSkipCount <= SKIP_COOLDOWN_SESSION_THRESHOLD) {
        // A küszöbig csak egy rövid, néma szünet — nincs visszaszámláló-szöveg, csak annyi,
        // hogy ne lehessen egyetlen pillanat alatt többször egymás után kattintani.
        setTimeout(() => {
            if (myToken !== skipCooldownToken) return; // közben új kör indult
...
function endRound(isWin, msgText, isFlawless = false) {
    ...
    if (isWin) {
        ...
        if (isFlawless) { modalBox.classList.add('flawless-state'); vibrate(HAPTIC_PATTERNS.flawless); }
        else { modalBox.classList.add('win-state'); vibrate(HAPTIC_PATTERNS.win); }
    } else {
        stats.wrong++;
        currentStreak = 0;
        streakDisplay.classList.add('hidden');
        modalBox.classList.add('lose-state');
        vibrate(HAPTIC_PATTERNS.lose);
    }
    ...
    const outcome = !isWin ? 'lose' : (isFlawless ? 'flawless' : 'win');
    setTimeout(() => {
        modalOverlay.classList.add('show');
        triggerRevealAnimation(resultCover, resultCoverWrap, modalBox, outcome);
    }, 300);
}
...
function showStats() {
    ...
    // Gyakorló módban SEMMIT nem mentünk el — sem a profilt (XP, jelvények, statisztika,
    // Kihagyás-arány), sem a ranglistát nem érinti ez a menet. Csak a kör-XP-t számoljuk ki
    // (megjelenítéshez), hogy a játékos lássa, mennyit ért volna, ha nem gyakorló módban
    // játszik.
    if (!config.practiceMode) {
        ...
        // Zenei ízlésprofil: csak akkor tudjuk egyértelműen egy kategóriához kötni a kör
        // eredményét, ha PONTOSAN egy kategória volt kiválasztva ehhez a menethez — több
        // egyszerre kiválasztott kategóriánál nem lehet tudni, melyik konkrét kör melyik
        // kategóriából jött, ezért azokat a meneteket kihagyjuk ebből a statisztikából.
        const catKeysForStats = Array.isArray(config.category) ? config.category : [config.category];
        if (catKeysForStats.length === 1) {
            const soloCatKey = catKeysForStats[0];
            if (!profile.categoryStats) profile.categoryStats = {};
            if (!profile.categoryStats[soloCatKey]) profile.categoryStats[soloCatKey] = { correct: 0, total: 0 };
            profile.categoryStats[soloCatKey].correct += stats.correct;
            profile.categoryStats[soloCatKey].total += config.rounds;
        }
        ...
        skipMultiplier = skipRateMultiplier(profile);
        ...
        // módosítása" gombja (és a lefoglalási rendszer) változtathatja meg — az egyéni
        // játék mezője (config.username, ami bejelentkezve amúgy is zárolva/előre kitöltve
        // van, lásd applyUsernameInputLockState) nem írhatja felül.
        if (!currentUser || currentUser.isAnonymous) {
            profile.username = config.username;
        }

        newBadges = checkAndAwardBadges(profile, eventFlags);
        saveProfile(profile);
        levelAfter = getLevelInfo(profile.xp).level;
    } else {
        roundResults.forEach(r => { sessionXp += xpForRound(r.win, r.flawless, r.step); });
    }

    // --- UI render ---
    statsTitle.textContent = config.isDaily ? "NAPI KIHÍVÁS" : (config.practiceMode ? "GYAKORLÓ MÓD" : "JÁTÉK VÉGE");
    ...
    statXp.textContent = config.practiceMode
        ? `🧪 Gyakorló mód — ${sessionXp} XP-t érne, de nincs mentve`
        : `+${sessionXp} XP` + (skipMultiplier < 1 ? ` (⚠️ -${Math.round((1 - skipMultiplier) * 100)}% a gyakori Kihagyás miatt)` : '');
...
shareResultBtn.addEventListener('click', async () => {
    ...
    if (ok) bouncePop(shareResultBtn);
});
...
// Zenei ízlésprofil — kategóriánkénti pontossági sáv, a legtöbbet játszott kategóriától
// a legkevésbé játszottig rendezve (ez adja a legjobb képet arról, mi a "fő terepe" a
// játékosnak). Csak azok a kategóriák szerepelnek, amikben tényleg volt legalább egy
// egyértelműen hozzárendelhető kör (lásd showStats / multiplayer.js renderMpResults).
function renderTasteProfile(profile) {
    const stats = profile.categoryStats || {};
    const entries = Object.keys(stats)
        .map(key => ({ key, correct: stats[key].correct || 0, total: stats[key].total || 0 }))
        .filter(e => e.total > 0)
        .sort((a, b) => b.total - a.total);

    if (entries.length === 0) {
        tasteProfileArea.innerHTML = '<p class="taste-profile-empty">Játssz néhány kört EGYETLEN kategóriában, hogy itt megjelenjen az ízlésprofilod!</p>';
        return;
    }

    tasteProfileArea.innerHTML = entries.map(e => {
        const pct = Math.round((e.correct / e.total) * 100);
        const label = `${CATEGORY_ICONS[e.key] || '🎵'} ${CATEGORY_LABELS[e.key] || e.key}`;
        return `<div class="taste-profile-row">
            <span class="taste-profile-label">${escapeHtml(label)}</span>
            <div class="taste-profile-bar-track"><div class="taste-profile-bar-fill" style="width:${pct}%"></div></div>
            <span class="taste-profile-pct">${e.correct}/${e.total} · ${pct}%</span>
        </div>`;
    }).join('');
}
...
// ==========================================
// 14b. INTERAKCIÓS ANIMÁCIÓK (gomb-hullám, csúszó szegmentált "pill")
// ==========================================
// Egyetlen, DELEGÁLT eseményfigyelő az egész dokumentumon — ez automatikusan lefedi az
// ÖSSZES jelenlegi ÉS jövőbeli .btn elemet is (nem kell mindegyikre külön feliratkozni).
// Billentyűzettel aktivált kattintásnál (pl. Enter/Space egy fókuszált gombon) a
// clientX/clientY 0, ilyenkor a gomb közepéről indítjuk a hullámot.
...
// A szegmentált (radio-alapú) vezérlők (Játékmód, Bejelentkezés/Regisztráció, Ranglista
// Globális/Helyi stb.) mögé egy csúszó "pill" hátteret helyezünk, ami a két opció között
// ÁTÚSZIK ahelyett, hogy csak egyik helyről a másikra ugrana a szín.
function initSegmentedPill(container) {
    if (!container) return;
    ...
    function reposition() {
        ...
        if (!label) { pill.style.opacity = '0'; return; }
        pill.style.opacity = '1';
        pill.style.width = `${label.offsetWidth}px`;
        pill.style.transform = `translateX(${label.offsetLeft}px)`;
    }

    container.querySelectorAll('input').forEach(input => input.addEventListener('change', reposition));
    window.addEventListener('resize', reposition);
    requestAnimationFrame(reposition);
    // A fenti azonnali mérés még a betöltés alatt lévő webfont (Inter, Google Fonts) HELYETT
    // egy tartalék rendszerbetűtípussal számolhat — ha utána becserélődik a valódi betűtípus,
    // a szöveg szélessége (és ezzel a label mérete) picit változhat, de a pill mérete/pozíciója
    // enélkül a régi, elavult méréshez ragadna, és láthatóan elcsúszna a gombtól. A
    // document.fonts.ready később, a tényleges betöltés után újra lefuttatja a mérést.
    if (document.fonts && document.fonts.ready) {
        document.fonts.ready.then(reposition).catch(() => {});
    }
}
['mode-segmented', 'auth-mode-segmented', 'leaderboard-scope-segmented', 'mp-mode-segmented', 'difficulty-segmented', 'mp-difficulty-segmented']
    .forEach(id => initSegmentedPill(document.getElementById(id)));
