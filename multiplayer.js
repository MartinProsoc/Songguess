...
const mpRoundStartCountdownOverlay = document.getElementById('mp-round-start-countdown');
const mpRoundStartCountdownNumber = document.getElementById('mp-round-start-countdown-number');
...
const mpRevealCard = document.getElementById('mp-reveal-card');
const mpRevealCover = document.getElementById('mp-reveal-cover');
const mpRevealTitle = document.getElementById('mp-reveal-title');
...
// A "szívverés" (lásd startMpHeartbeat) 4 másodpercenként ÚJRA lekérdezi a szobát/játékosokat
// (és minden résztvevő lastSeen-írása is kivált egy onSnapshot-frissítést) — enélkül a jelzés
// nélkül ez korábban MINDEN egyes ilyen alkalommal újraépítette a teljes játékoslistát
// (innerHTML csere), ami a neveket folyamatosan felvillanni/újra-animálódni láttatta, még
// akkor is, ha ténylegesen semmi nem változott. Ezekkel a "signature" gyorsítótárakkal csak
// akkor építjük újra a listát, ha a benne lévő adat (név/ready/pontszám/sorrend) tényleg más.
let mpLobbyListSignature = null;
let mpLiveLbSignature = null;
let mpWaitingListSignature = null;
...
// Vendég (nem bejelentkezett) vagy akár bejelentkezett, de MÁS néven szereplő játékos ne
// tudja csendben felvenni egy MÁSIK, már regisztrált fiók által lefoglalt nevét — ez a
// "usernames" gyűjteményt (lásd script.js -> claimUsername) használja fel, amit bárki
// hitelesített (akár névtelen) felhasználó olvashat a firestore.rules szerint. Ha ütközés
// van, egy jelzést fűzünk a névhez, hogy a szobában egyértelműen megkülönböztethető legyen
// a valódi fiók a vendégtől.
async function resolveNonConflictingName(rawName, myUid) {
    const fallback = (rawName || '').trim() || 'Játékos';
    if (typeof firestoreDb === 'undefined' || !firestoreDb || typeof normalizeUsernameKey !== 'function') return fallback;
    try {
        const snap = await firestoreDb.collection('usernames').doc(normalizeUsernameKey(fallback)).get();
        if (snap.exists && snap.data().uid !== myUid) return fallback + ' (Vendég)';
    } catch (e) {
        // Olvasási hiba esetén inkább engedjük át a nevet, mint hogy emiatt ne lehessen
...
mpCreateConfirmBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const config = {
            ...
            difficulty: document.querySelector('input[name="mp-difficulty"]:checked').value,
            maxPlayers: parseInt(mpMaxPlayersInput.value) || 4
        };
        const name = hostDisplayNameFromSetup();
        if ((await resolveNonConflictingName(name, user.uid)) !== name) {
            mpSetupErrorMsg.textContent = 'Ezt a nevet már foglalta egy regisztrált fiók — válassz másik nevet.';
...
async function joinRoomByCode(code, user, errorEl) {
    ...
    if (!myExisting.exists) {
        ...
        const name = await resolveNonConflictingName(currentMpDisplayName(), user.uid);
...
mpQuickplayBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (!joined) {
            ...
            const config = { category: ['all'], mode: 'classic', rounds: 5, timeLimit: 30, hardcore: false, difficulty: 'any', maxPlayers: 6 };
            const name = await resolveNonConflictingName(currentMpDisplayName(), user.uid);
            let code = null;
            for (let attempt = 0; attempt < 6; attempt++) {
                const candidate = generateRoomCodeCandidate();
...
function enterRoom(code, isHost) {
    ...
    mpAdvancingRound = 0;
    mpHostClaimAt = 0;
    mpLobbyListSignature = null;
    mpLiveLbSignature = null;
    mpWaitingListSignature = null;
...
function renderLobby() {
    ...
    const myUid = mpMyUid();
    const listSignature = players.map(p => `${p.id}:${p.name}:${p.ready ? 1 : 0}:${p.id === mpRoomData.hostUid ? 1 : 0}`).join('|');
    if (listSignature !== mpLobbyListSignature) {
        mpLobbyListSignature = listSignature;
...
mpReadyBtn.addEventListener('click', () => {
    ...
    mpReadyBtn.classList.remove('combo-pop');
    void mpReadyBtn.offsetWidth;
    mpReadyBtn.classList.add('combo-pop');
});
...
async function beginMultiplayerRound() {
    ...
    try {
        ...
        for (let i = 0; i < cfg.rounds; i++) {
            const song = await getRandomSongFromiTunes(cfg.category, usedIds, Math.random, cfg.difficulty);
            ...
            usedIds.add(song.id);
            usedIds.add(normalizeSongIdentity(song.artist, song.title));
            songs.push(song);
            if (cfg.mode === 'choice') {
...
function startMpRound(roundNum) {
    ...
    // "3... 2... 1..." visszaszámláló végén nyílnak csak meg (finalizeMpRoundStart), hogy
    // egy résztvevő se tudjon előnyt szerezni a visszaszámlálás alatt. Mivel ezt a Firestore
    // pillanatkép indítja el, minden játékos nagyjából egyszerre látja a visszaszámlálót.
    ...
    playRoundStartCountdown(mpRoundStartCountdownOverlay, mpRoundStartCountdownNumber, () => finalizeMpRoundStart(cfg));
}

function finalizeMpRoundStart(cfg) {
    mpPlayBtn.disabled = false;
    mpSkipBtn.disabled = !!cfg.hardcore;
    mpSubmitBtn.disabled = false;
    mpGuessInput.disabled = false;
    mpChoiceInputArea.querySelectorAll('.choice-btn').forEach(b => b.disabled = false);
...
function triggerMpSkipCooldown() {
    ...
    if (mpSessionSkipCount <= SKIP_COOLDOWN_SESSION_THRESHOLD) {
        // A küszöbig csak egy rövid, néma szünet — enélkül egy körön belül szinte azonnal,
        // egymás után kattintva végig lehetett ugrani a hangrészlet-fokozatokon, tisztességtelen
        // előnyt adva ahhoz képest, amíg a többiek épp csak az első, rövid részletet hallják.
        setTimeout(() => {
            if (myToken !== mpSkipCooldownToken) return; // közben új kör indult
...
function endMpRound(isWin, msgText, isFlawless = false) {
    ...
    if (isWin) {
        ...
        if (mpMyStreak > 0) {
            ...
            mpStreakDisplay.classList.remove('combo-pop');
            void mpStreakDisplay.offsetWidth;
            mpStreakDisplay.classList.add('combo-pop');
        } else {
            mpStreakDisplay.classList.add('hidden');
        }
        mpMessageDisplay.style.color = 'var(--accent-2)';
        vibrate(isFlawless ? HAPTIC_PATTERNS.flawless : HAPTIC_PATTERNS.win);
    } else {
        mpMyStreak = 0;
        mpStreakDisplay.classList.add('hidden');
    ...
    mpMessageDisplay.textContent = msgText;

    // Válasz-felfedő kártya — ugyanaz az animáció-készlet (reveal-pop / reveal-shake /
    // szikrák), mint az egyéni játék eredmény-modáljában, lásd script.js
    // triggerRevealAnimation().
    const outcome = !isWin ? 'lose' : (isFlawless ? 'flawless' : 'win');
    mpRevealCard.className = 'reveal-card' + (outcome === 'lose' ? ' lose-state' : outcome === 'flawless' ? ' flawless-state' : ' win-state');
    mpRevealCard.classList.remove('hidden');
    mpRevealCover.onerror = function () { this.onerror = null; this.src = fallbackImg; };
    mpRevealCover.src = mpCurrentSong.cover;
    mpRevealTitle.textContent = `${mpCurrentSong.artist} - ${mpCurrentSong.title}`;
    triggerRevealAnimation(mpRevealCover, mpRevealCover.parentElement, mpRevealCard, outcome);
    ...
    setTimeout(() => {
        mpMessageDisplay.textContent = '';
        mpRevealCard.classList.add('hidden');
...
function renderWaitingList() {
    ...
    if (text !== mpWaitingListSignature) {
        mpWaitingListSignature = text;
...
function renderLiveLeaderboard() {
    ...
    const signature = players.map(p => `${p.id}:${p.name}:${p.score || 0}`).join('|');
    if (signature === mpLiveLbSignature) return;
    mpLiveLbSignature = signature;
...
function renderMpResults() {
    ...
    const mpCatKeysThisGame = Array.isArray(mpRoomData.config.category) ? mpRoomData.config.category : [mpRoomData.config.category];
    mpCatKeysThisGame.forEach(catKey => {
        if (!profile.categoriesPlayed.includes(catKey)) profile.categoriesPlayed.push(catKey);
    });
    // Zenei ízlésprofil — csak akkor tudjuk egyértelműen egy kategóriához kötni, ha PONTOSAN
    // egy kategória volt kiválasztva a szobában (lásd script.js showStats ugyanezt a logikát).
    if (mpCatKeysThisGame.length === 1) {
        const mpSoloCatKey = mpCatKeysThisGame[0];
        if (!profile.categoryStats) profile.categoryStats = {};
        if (!profile.categoryStats[mpSoloCatKey]) profile.categoryStats[mpSoloCatKey] = { correct: 0, total: 0 };
        profile.categoryStats[mpSoloCatKey].correct += correctCount;
        profile.categoryStats[mpSoloCatKey].total += myResults.length;
    }
...
// ==========================================
// 17. LINK ALAPÚ MEGHÍVÁS (a kód előtöltése — csatlakozás csak KATTINTÁSRA)
// ==========================================
// Ha valaki a "🔗 Link másolása" gombbal kapott linket nyitja meg (pl. ...?room=A7K2XQ),
// előtöltjük neki a kódot, hogy ne kelljen begépelnie — de a TÉNYLEGES csatlakozás
// (Firebase-bejelentkezés + szoba-írás) KIZÁRÓLAG a "Csatlakozás" gomb kattintására indul el,
// sosem automatikusan az oldal betöltésekor.
//
// Ez direkt így lett megcsinálva (korábban automatikusan csatlakoztatott betöltéskor) — egy
// felhasználó jelezte, hogy Safari-n a linket megosztó alkalmazás (pl. iMessage/WhatsApp)
// linkelőnézet-generálás céljából LÁTHATATLANUL, a háttérben is betölti (lefuttatja a JS-t)
// a megosztott oldalt, hogy címet/képet tudjon mutatni — ez a saját, önálló névtelen
// Firebase-munkamenetével CSENDBEN csatlakozott volna a szobához, majd amikor a felhasználó
// ténylegesen megnyitotta a linket, egy MÁSODIK, valódi munkamenet is csatlakozott ugyanahhoz
// a szobához — innen a jelentett "duplikált/fantom játékos" (egy emberből kettő lett).
// Mivel egy láthatatlan előnézet-betöltés nem tud gombra kattintani, ez a megoldás kizárja
...
(function initRoomInviteLink() {
    ...
    mpShowScreen(mpEntryScreen);
    mpJoinCodeInput.value = code;
    mpEntryErrorMsg.textContent = `Meghívást kaptál a(z) ${code} szobához — nyomd meg a "Csatlakozás" gombot!`;
})();
